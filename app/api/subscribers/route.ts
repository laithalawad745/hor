...
import { initializeUnifiedBot } from '@/lib/telegram-bot-manager'  // ✅ التحديث
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // ✅ تهيئة البوت في وضع Webhook للـ Production
      const isProduction = process.env.NODE_ENV === 'production'
      const bot = initializeUnifiedBot(isProduction)
      ...
      if (bot && bot.bot) {
        ...
        if (existingSubscriber) {
          ...
          if (wasActive) {
            // المشترك كان نشطاً - نرسل رسالة تمديد
            try {

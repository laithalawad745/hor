...
export default function Payment() {
  ...
  // معلومات الدفع
  const usdtAddress = "TNyPBsRhoXsWAoNDmcfkkx1ku5Bn3ofGmY"
  const paypalEmail = "gammaltechstudent@gmail.com"
  const telegramUsername = "@Telecop_support"
  
  // خيارات الأسعار
  const priceOptions = [
    { value: '20', label: '$20', description: language === 'ar' ? 'خطة أساسية' : 'Basic Plan' },
    { value: '50', label: '$50', description: language === 'ar' ? 'خطة متوسطة' : 'Standard Plan' },
    { value: '100', label: '$100', description: language === 'ar' ? 'خطة احترافية' : 'Professional Plan' }
  ]
...
const copyToClipboard = async (text: string, type: 'address' | 'amount') => {
  try {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      ...
      // حل بديل للأجهزة القديمة
      const textArea = document.createElement("textarea")
      textArea.value = text
      textArea.style.position = "fixed" // يمنع القفز بالصفحة
  ...
  <input
    ...
    value={usdtAddress}
    readOnly
    className="w-full bg-transparent text-sm font-mono text-gray-300 outline-none"
    dir="ltr"
  />

  <button
    onClick={() => copyToClipboard(usdtAddress, 'address')}
    className="flex items-center gap-2 px-3 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg transition-colors w-full sm:w-auto justify-center"
  >
...
<div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
  <span className="text-lg font-medium break-all" dir="ltr">{paypalEmail}</span>
  <button
